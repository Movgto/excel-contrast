...
def main():
    ...
    sheet_mapping_obs = Signal[dict]({})

    sheet_validation_comp(sheet_val_cont, filepath1=route_a, filepath2=route_b, validate_btn=validate_sheets_btn, sheets_obs=sheets_dict_obs, sheet_mapping_obs=sheet_mapping_obs)
    ...
    def on_compare_btn_click():
        ...
        result = check_diferences(route_a.get(), route_b.get(), sheets_dict_obs.get(), prog_bar_obs=progress, sheet_mapping=sheet_mapping_obs.get())
        ...
        for sheet_name in result.keys():    
            file_path = os.path.join(dir, f'result_{sheet_name}_{date_str}.xlsx')

            print('Probably this is the index column of the sheet comparison:')        
            print(result[sheet_name].index)

            result[sheet_name].index = result[sheet_name].index + 2
...
if __name__ == '__main__':
    try:
        ...
        print(f"\n{'='*60}")
        print(f"❌ ERROR EN LA APLICACIÓN:")
        print(f"{'='*60}")
